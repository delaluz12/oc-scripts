...
async function run() {
    const creds = config.seb.staging.europaSports
    ...
    // divisor
    const resaleDivisor = 1.03 // Resale products
        ...
        try {
            let patchObject = variant.Variant.xp?.SpecValues.map(spec => {
                if (spec.PriceMarkup != null && spec.PriceMarkup != 0) {
                    const newMarkup = Number(spec.PriceMarkup) / resaleDivisor
    ...
    await helpers.batchOperations(specsToUpdate, async function singleOperation(spec): Promise<any> {
        try {
            let newMarkup: number | undefined = Number(spec.PriceMarkup!) / resaleDivisor
